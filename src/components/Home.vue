...
export default {
  ...
  methods: {
    ...
    async getTest(tweet_id) {
      fetch('http://127.0.0.1:3000/interaction/like', {
        method: 'GET',
        headers: {
          tweet_id: tweet_id
        },
...
<template>
  <router-link to="/newTweet">New Tweet</router-link>
      ...
      <div class="flex sm:w-full bg-opacity-15 bg-blue-300 md:mb-0 p-5 shadow-xl rounded-xl mr-3 ml-3">
        <div class="relative w-full text-left">
          <p class="text-md text-gray-800 text-left leading-normal mb-5 font-lf-normal">
              <p class="text-sm text-gray-600">{{ t.text }}</p>
          </p>
...
<!--                          <div v-if="haveLike"></div>-->
            ...
            {{t.nb_reply}}
                        <a>
                          <img style="width: 30px" src="/reply.png">
                        </a>
          </div>
